...
public class SubjectExcelListener extends AnalysisEventListener<SubjectData> {
    ...
    private EduSubject existOneSubject(EduSubjectService subjectService, String name) {
        ...
        wrapper.eq("title",name);           //name = 前端开发
        wrapper.eq("parent_id","0");   //parent_id = 0
        EduSubject oneSubject = subjectService.getOne(wrapper);//只取一条
    ...
    private EduSubject existTwoSubject(EduSubjectService subjectService,String name,String pid) {
        ...
        EduSubject twoSubject = subjectService.getOne(wrapper); //投机取巧
        return twoSubject;
    }
    ...
    public void invoke(SubjectData subjectData, AnalysisContext analysisContext) {
        ...
        if(existOneSubject == null) { //没有相同一级分类，进行添加
            existOneSubject = new EduSubject();//EduSubject即可以是一级，也可以二级
